...
- [bepoptimist](https://github.com/sheoak/zsh-bepoptimist/) 重新映射法语的 vi 模式 [bépo](http://bepo.fr/wiki/Accueil) 键盘.
...
- [cmdtime](https://github.com/tom-auger/cmdtime) - 显示命令到终端的持续时间 [timer](https://github.com/ohmyzsh/ohmyzsh/tree/master/plugins/timer) plugin.
...
- [dropbox](https://github.com/zpm-zsh/dropbox) - 一个 [dropbox](https://www.dropbox.com/) plugin for ZSH that provides `dropbox-cli` and `dropbox-uploader` commands.
...
- [fuzzy-wd](https://github.com/spodin/zsh-fuzzy-wd) - 为扭曲的目录添加模糊搜索 [WD](https://github.com/ohmyzsh/ohmyzsh/blob/master/plugins/wd) plugin.
...
- [git-ignore](https://github.com/laggardkernel/git-ignore) - 从 gitignore.io **offline** 生成 `.gitignore` 文件. [fzf](https://github.com/junegunn/fzf), completion, preview integrated.
...
- [mysql-login](https://github.com/remino/omz-plugin-mysql-alias) - 使用登录路径为 MySQL 添加别名.
...
- [npm (igoradamenko)](https://github.com/igoradamenko/npm.plugin.zsh)  - 添加“npm”别名和命令补全. 基于 Oh-My-Zsh [npm](https://github.com/ohmyzsh/ohmyzsh/tree/master/plugins/npm) plugin.
...
- [pseudo-projectile](https://github.com/git-girl/pseudo-projectile-plugin/)  - 像emacs中的`projectile`？ 这通过在 `nvim` 中打开 `fzf`-selected 路径为您的 ZSH 环境添加了类似的功能.
...
- [pyenv (twopizza9621536)](https://github.com/TwoPizza9621536/zsh-pyenv) - 基于 oh-my-zsh [pyenv](https://github.com/ohmyzsh/ohmyzsh/tree/master/plugins/pyenv) 带有 rbenv 和 jenv 插件修改的插件.
...
- [singularityenv](https://github.com/saravanabalagi/zsh-plugin-singularityenv) - 提供 `singularityenv_prompt_info` 函数，该函数返回当前的奇异环境名称
...
- [spaceship-ocm](https://github.com/iamkirkbater/spaceship-ocm-plugin)  - 查询您的 OpenShift 集群管理器 (ocm) 配置以显示您连接到的环境. 需要 [NerdFont](https://www.nerdfonts.com/font-downloads) 在您的终端中.
...
- [svn-n-zsh](https://github.com/khrt/svn-n-zsh-plugin) - 重写股票 [oh-my-zsh](https://ohmyz.sh/) [svn](https://subversion.apache.org/) plugin.
...
- [travis](https://github.com/denolfe/zsh-travis) - 打开 [Travis CI](https://www.travis-ci.com/) page for the current repo if one exists.
...
- [which-jspm](https://github.com/zkuzmic/which-jspm/) - 根据在当前目录中检测到的锁定文件，将 `npm`、`yarn` 或 `pnpm` 添加到提示符的末尾.
...
- [zbrowse](https://github.com/zdharma-continuum/zbrowse) - 在做shell工作时，经常会出现多次调用`echo $variable`，检查循环结果等情况.使用ZBrowse，你只需要按`Ctrl-B`，调用ZBrowse – Zshell 变量浏览器.
...
- [zshmux](https://github.com/huboles/zshmux)  - 当一个新的 shell 启动时，快速检查是否有任何 `tmux` 会话，或者当前 shell 是否已经在一个 `tmux` 实例中. 如果您还没有在 `tmux` 会话中，它将列出所有当前活动的会话，并询问您是否要附加当前正在运行的会话，或者在 shell 中创建一个新会话.
...
- [fancy-completions](https://github.com/z-shell/zsh-fancy-completions) - 提供各种完成工具、库和集成.
...
- [lets-cli](https://github.com/lets-cli/lets-zsh-plugin) - 添加自动完成 [lets](https://github.com/lets-cli/lets) cli 任务运行器.
...
- [mcfly](https://github.com/cantino/mcfly)  - 将您的默认 ctrl-r shell 历史搜索替换为智能搜索引擎，该引擎考虑您的工作目录和最近执行的命令的上下文.  McFly 的建议通过小型神经网络实时优先处理.
...
- [openstack](https://github.com/florentinl/openstack-zsh-plugin) - 添加管理功能和别名 [OpenStack](https://www.openstack.org/).
...
- [symfony-complete](https://github.com/voronkovich/symfony-complete.plugin.zsh) - 通用完成 [Symfony](https://symfony.com/doc/current/components/console.html) 基于 CLI 应用程序：`composer`、`php-cs-fix`、`bin/console`、`artisan`、`php-cs-fixer` 等.它支持子命令和 GNU 样式选项的自动完成（`--帮助`)
...
- [wsl-notify](https://github.com/masonc15/wsl-notify-zsh) - 用途 [wsl-notify-send](https://github.com/stuartleeks/wsl-notify-send) 当命令花费超过 15 秒时发出通知. 仅限 Windows.
...
- [amplify](https://github.com/clintfoster/ohmyzsh-theme-amplify) - 极简主义，包括 AWS Amplify 环境和 `git` 状态装饰.- [andy](https://github.com/andymcguinness/andys-theme) - Modified [bira](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/bira.zsh-theme) 具有更好的 `git` 支持的主题.
...
- [astral](https://github.com/xwmx/astral)  - 带有禅宗模式的深色背景主题. 适用于 zsh-users [zsh-syntax-highlighting](https://github.com/zsh-users/zsh-syntax-highlighting) 插入. 包括最后一个命令的执行时间、运行时间、退出状态、机器名、当前路径、`ssh` 状态和`git` 状态的装饰器.
...
- [bashplus](https://github.com/Elagoht/BashPlusZshTheme) - 默认 bash 提示的彩色副本，带有 virtualenv 和 `git` 状态的装饰器.
...
- [bubblified (varaki)](https://github.com/varaki/bubblified-varaki.zsh-theme) - 基于 [bubblified (hohmannr)](https://github.com/hohmannr/bubblified) . 生根时变色. 包括装饰器以显示 user@host 和当前目录.
...
- [carriage-return](https://github.com/treyssatvincent/carriage-return.zsh-theme) - omz [robbyrussell](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/robbyrussell.zsh-theme) 加上回车.
...
- [clean (brandonRoehl)](https://github.com/BrandonRoehl/zsh-clean) - A minimalist variant of [pure](https://github.com/sindresorhus/pure) . 清净不净，净不净.
...
- [cravend](https://github.com/cravend/theme) - 包括 `hostname` 装饰器（仅在活动的 `ssh` 会话中）和 `git` 状态装饰.
...
- [dangerroom](https://github.com/abbreviatedman/dangerroom)  - 内容丰富、极简，最重要的是，以 X 战警为主题. 包括 `git` 状态、工作目录、父目录和 `vim` 模式的装饰器.
...
- [funkydrac](https://github.com/warshanks/funkydrac) - 多个以德古拉为主题的 omz 主题，基于 [funky](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/funky.zsh-theme) 和 [oh-my-posh](https://github.com/JanDeDobbeleer/oh-my-posh) 主题基于 [alien](https://github.com/JanDeDobbeleer/oh-my-posh/blob/main/themes/aliens.omp.json)
...
- [happy-coding](https://github.com/lexhuismans/happy-coding/) - 精简版 [passion](https://github.com/ChesterYue/ohmyzsh-theme-passion) . 包括时间、`git` 分支、最后一个命令执行时间和最后一个命令退出状态的装饰器.
...
- [humbled](https://github.com/saravanabalagi/zsh-theme-humbled)  - 一个干净而谦逊的主题，具有左对齐的 `condaenv`、`virtualenv` 和 `git` 状态. 需要 [condaenv](https://github.com/saravanabalagi/zsh-plugin-condaenv) plugin.
...
- [kawaii](https://github.com/LeonidPilyugin/kawaii-oh-my-zsh/)  - 具有终端和虚拟控制台模式. 包括用户名、目录、最后一个命令退出状态、时间戳和 `git` 状态的装饰器.
...
- [lpha3cho](https://github.com/sdcampbell/lpha3cho-Oh-My-Zsh-theme-for-pentesters) - 修改版 [intheloop](https://github.com/ohmyzsh/ohmyzsh/blob/master/themes/intheloop.zsh-theme) 渗透测试者的主题，其中包括渗透测试日志记录的日期、时间和 IP 地址.
...
- [modesty](https://github.com/saravanabalagi/zsh-theme-modesty)  - 简洁的 ZSH 主题，带有 `condaenv`、`virtualenv` 和 `git` 状态装饰，整齐地右对齐显示. 需要 [condaenv](https://github.com/saravanabalagi/zsh-plugin-condaenv) plugin.
...
- [nanika](https://github.com/justforuse/nanika-zsh-theme/) - 优化的变体 [robbyrussell](https://github.com/ohmyzsh/ohmyzsh/wiki/Themes#robbyrussell) . 包括 `git` 状态装饰.
...
- [nt9](https://github.com/lenguyenthanh/nt9-oh-my-zsh-theme)  - 一个干净、无干扰且以“git”为重点的开发主题. 显示相对于 `git` 根目录（或在 `git` 存储库外部时为 `~`）、自上次提交以来的时间、当前 SHA、分支和分支状态.
...
- [searocket](https://github.com/dk949/searocket/) - 瘦身版 [spaceship](https://github.com/denysdovhan/spaceship-prompt) . 包括工作目录、最后命令退出代码、用户、后台作业、`bun`、`d`、elm、go、nodejs、python、zig 和 `git` 状态的装饰器.
...
- [windows](https://github.com/juliavallina/windows-zsh-theme/)  - 灵感来自 Windows 命令提示符. 包括当前目录的装饰器.
...
- [ys](https://github.com/cristiancavalli/ys-zsh-custom-theme) - 干净、简单、兼容和有意义的主题，适用于深色背景.
...
### [Zgen](https://github.com/tarjoilija/zgen)
 ...
 Zgen 没有得到积极维护. 我建议你切换到 [Zgenom](https://github.com/jandamm/zgenom) 叉子，也就是.
...
非常感谢多年来所有的贡献者. 如果没有您的帮助，这份清单就不会那么完整.
