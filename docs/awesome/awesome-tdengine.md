...
时间序列数据的开源、高性能简化解决方案
 ...
 TDengine 是一个开源的云原生时间序列数据库 (TSDB)，针对物联网 (IoT)、联网汽车和工业物联网进行了优化. 它支持每天高效、实时地摄取、处理和监控由数十亿传感器和数据收集器生成的 PB 级数据. 凭借其内置的缓存、流处理和数据订阅功能，TDengine 为时间序列数据处理提供了一个简化的解决方案.
