...
- [ArcGIS Runtime API for iOS](https://developers.arcgis.com/ios/) - 使用 Swift 或 Objective-C 构建移动应用程序的指南、示例代码和 API 参考.
- [ArcGIS Runtime API for Java](https://developers.arcgis.com/java/) - 构建桌面应用程序的指南、示例代码和 API 参考.
- [ArcGIS Runtime API for Qt](https://developers.arcgis.com/qt/) - 用于构建移动和桌面应用程序的指南、示例代码和 API 参考.
...
- [GeoEnrichment service](https://developers.arcgis.com/documentation/mapping-apis-and-services/demographics/services/geoenrichment-service/) - 查找有关位置或区域的事实和人口统计信息.
- [Routing service](https://developers.arcgis.com/documentation/mapping-apis-and-services/routing/services/routing-service/) - 获取转弯指示并解决高级路线问题.
...
- [ArcGIS Runtime API for Android Kotlin samples](https://developers.arcgis.com/android/kotlin/sample-code/) - Esri 的 Kotlin 官方 ArcGIS Runtime Android 产品团队示例.
...
- [ArcGIS Runtime API for Qt C++ sample code](https://developers.arcgis.com/qt/cpp/sample-code/) - Esri 的 C++ 官方 ArcGIS Runtime Qt 产品团队示例.
- [ArcGIS Runtime API for Qt QML sample code](https://developers.arcgis.com/qt/qml/sample-code/) - Esri 的 QML 官方 ArcGIS Runtime Qt 产品团队示例.
...
- [Generalize method](https://bit.ly/2VNPkuO)  - GeometryEngine 可以以编程方式生成具有较少顶点的几何图形. 有几个 API 支持它：JavaScript、iOS、Android、.NET、Qt 和 Java.
...
- [geojsonio](https://github.com/ropensci/geojsonio) - 在 GeoJSON 和 TopoJSON 之间转换多种数据格式.
- [gtfs2geojson](https://github.com/node-geojson/gtfs2geojson) - 将 GTFS 数据转换为 GeoJSON.
- [Koop](https://koopjs.github.io)  - 用于连接空间 API 的 JavaScript 工具包. 即时转换地理空间数据并用作 GeoJSON、矢量切片、要素服务等.
...
- [terraformer](https://github.com/terraformer-js/terraformer) - 将 ArcGIS JSON 与 GeoJSON 相互转换，将 WKT 几何图形与 GeoJSON 几何图形相互转换，以及其他格式.
- [togeojson](https://mapbox.github.io/togeojson/) - 毫不费力地将 KML 和 GPX 转换为 GeoJSON.
...
- [Integromat](https://www.integromat.com/en/integrations/survey123) - iPaaS 可自动执行使用 Survey123 所涉及的重复性任务，让您的工作更轻松.
- [node-red-contrib-arcgis-rest](https://flows.nodered.org/node/node-red-contrib-arcgis-rest) - 使用 JS 基金会的事件驱动应用程序的低代码编程查询、删除、更新或插入数据.
- [Zapier for ArcGIS](https://marketplace.arcgis.com/listing.html?id=5ab7936269f8449b82b0f5c78695ab38) - iPaaS 无需编写任何代码即可自动化集成.
- [Tray.io](https://tray.io/connectors/arcgis-integrations) - 使用 Tray Platform 的 ArcGIS 连接器手动、计划和 webhook 触发器以应用编辑、获取要素、图层等.
...
- 最佳实践、书籍、视频和培训：
	...
	- [Photoshop-style Graphics Effects for Your Layers and Data](https://www.youtube.com/watch?v=crmWm80hwKI) - 视频解释了如何使用混合模式以及图层和要素效果来使用 ArcGIS API for JavaScript 4.x 创建独特而令人惊叹的 web 地图. 
...
- 图形用户界面：
	- [ArcGIS Vector Tile Style Editor](https://developers.arcgis.com/documentation/mapping-apis-and-services/tools/vector-tile-style-editor/) - 应用程序的样式矢量平铺底图图层.
...
- [Search](https://developers.arcgis.com/documentation/mapping-apis-and-services/search/) - 使用地理编码服务搜索地址、企业和景点 (POI).
...
- [ArcGIS Scene Viewer](https://www.arcgis.com/home/webscene/viewer.html) - 为 3D 应用程序创建、探索和共享 Web 地图的 Web 应用程序.
...
- [Smart Mapping](https://www.esri.com/en-us/smart-mapping) - 它内置在地图和场景查看器中，但 JavaScript 和 Python 等一些 API 也提供实用程序来帮助构建数据探索工具.

## Playgrounds

- [arcgis-arcade-playground](https://developers.arcgis.com/arcade/playground/) - 尝试使用便携式脚本语言创建 ArcGIS 自定义可视化和标注表达式.
- [cim-symbol-builder](https://github.com/Esri/cim-symbol-builder-js) - 生成 CIM 符号以使用 ArcGIS 客户端 API 和要素服务.
...
- [Spatial reference specifications](https://developers.arcgis.com/documentation/spatial-references/#spatial-reference-specifications) - 已知 ID (WKID) 整数值列表或称为已知文本 (WKT) 的文本字符串定义，用于定义空间参考.
- [Tile Package Specification](https://github.com/Esri/tile-package-spec) - 包含一组切片和切片方案的压缩文件，可用作 ArcGIS 应用程序中的底图.
- [Web Map spec](https://developers.arcgis.com/web-map-specification/)  - 可共享的 2D 地图. 它描述了定义 web 地图的 JSON 对象.
- [Web Scene spec](https://developers.arcgis.com/web-scene-specification/) - 为可共享的 3D 场景定义内容（视点、相机、底图图层、图层、样式等）的 JSON 结构.

## Starter apps and CLIs

- [@arcgis/cli](https://github.com/Esri/arcgis-js-cli) - 为 ArcGIS API for JavaScript 快速搭建各种应用程序.
- [generator-esri-appbuilder-js](https://github.com/Esri/generator-esri-appbuilder-js) - Yeoman 生成器，帮助自定义 Esri 的 Web AppBuilder.
...
- [awesome-arcgis](https://github.com/esri-es/awesome-arcgis/) - 带有 wiki 风格的很棒的列表，其中包含有关 Esri 和 ArcGIS 的资源，按以下方式组织：产品、行业、文件格式、内容提供商等.
- [awesome-earthobservation-code](https://github.com/acgeospatial/awesome-earthobservation-code) - 关于地球观测和地理空间资料的工具、教程、代码、有用的项目和链接.
...
- [awesome-gis](https://github.com/sshuair/awesome-gis) - GIS、遥感、3D 应用程序、Web 地图服务器、地理空间库、开放标准、数据等.
- [awesome-json-datasets](https://github.com/jdorfman/awesome-json-datasets) - 不需要身份验证的 JSON 数据集：气候、犯罪、政府、NASA、旅行等.
- [awesome-open-geoscience](https://github.com/softwareunderground/awesome-open-geoscience) - 从使我们作为地球科学家、黑客和数据争吵者的生活更轻松或更棒的存储库中挑选出来.
...
- [awesome-semantic-segmentation](https://github.com/mrgloom/awesome-semantic-segmentation) - 网络架构（语义分割、实例感知分割等）、RNN、GANS、数据集等.
...
任何未解决的问题都是公平的游戏. 即使只是告诉我们您想看到的内容也会非常有帮助！
