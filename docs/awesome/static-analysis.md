...
- [Esprima](https://esprima.org) :warning: — 用于多用途分析的 ECMAScript 解析基础架构.
...
- [pure](https://github.com/ronomon/pure) :warning: — Pure 是一个静态分析文件格式检查器，可检查 ZIP 文件中是否存在危险的压缩率、规范偏差、恶意存档签名、本地和中央目录标题不匹配、不明确的 UTF-8 文件名、目录和符号链接遍历、无效的 MS-DOS日期、重叠标题、上溢、下溢、稀疏、意外缓冲区溢出等.
...
- [yamllint](https://yamllint.readthedocs.io) :warning: — 检查 YAML 文件的语法有效性、键重复和外观问题，例如行长、尾随空格和缩进.
